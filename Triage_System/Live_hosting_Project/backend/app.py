...
def _run_schema_on_startup():
    """Execute schema.sql against PostgreSQL on every boot. Idempotent — uses IF NOT EXISTS."""
    schema_path = _ROOT_DIR / "schema.sql"
    if not schema_path.exists():
        print("[startup] schema.sql not found, skipping schema run.")
        return
    try:
        from backend.db.connection import get_connection
        conn = get_connection()
        ...
        cursor.close()
        conn.close()
        print("[startup] schema.sql executed successfully.")
    except Exception as exc:
        print(f"[startup] Schema execution error: {exc}")
